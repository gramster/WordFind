...
class WordListSource : public ListSource
{
  ...
  public:
    ...
    int Start(const char *pattern, const char *anagram);
...
class WordList : public List
{
  ...
  public:
    ...
    int Start(const char *tmplate, const char *anagram)
    {
...
class WordForm : public Form
{
  ...
  public:
    ...
    int Start(const char *pattern, const char *anagram = 0) 
    {
...
int WordListSource::Start(const char *pattern, const char *anagram)
{
    int x = dict->StartConsult(pattern, anagram);
    nomore = (x != 0);
    return x;
}
...
void MainForm::Go()
{
    ...
    if (pattern[0] || anagram[0])
    {
        ...
        if (wordform)
        {
            int x = wordform->Start(pattern, anagram);
            if (x == 0)
    	        Switch(WordFormForm);
	    else if (x == -1)
